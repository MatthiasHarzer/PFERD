...
from .hka_ilias_html import (IliasElementType, IliasForumThread, IliasLearningModulePage, IliasPage,
                             IliasPageElement, _sanitize_path_name, parse_ilias_forum_export)

TargetType = Union[str, int]

_ILIAS_URL = "https://ilias.h-ka.de"
_ILIAS_LOGIN_URL = (f"{_ILIAS_URL}/ilias.php?lang=de&target=root_1&cmd=post&cmdClass=ilstartupgui&cmdNode=11g"
                    f"&baseClass=ilStartUpGUI&rtoken")


class HkaShibbolethBackgroundLoginSuccessful():
    pass


class HkaIliasWebCrawlerSection(HttpCrawlerSection):
...
class HkaIliasWebCrawler(HttpCrawler):
    def __init__(
            self,
            name: str,
            section: HkaIliasWebCrawlerSection,
            config: Config,
...
instance's greatest bottleneck.
        ...
        self._shibboleth_login = HkaShibbolethLogin(auth)
...
class HkaShibbolethLogin:
    ...
    async def _login_successful(sess: aiohttp.ClientSession) -> bool:
        url = f"{_ILIAS_URL}/ilias.php?cmdClass=ilmembershipoverviewgui&cmdNode=jq&baseClass=ilmembershipoverviewgui"

        async with sess.get(url) as request:
            soup = soupify(await request.read())
...
async def _shib_post(
        ...
        url: str,
        data: Any
) -> Union[BeautifulSoup, HkaShibbolethBackgroundLoginSuccessful]:
    """
    ...
    async with session.post(url, data=data, allow_redirects=False) as response:
        ...
        if str(correct_url).startswith(_ILIAS_URL):
            ...
            return HkaShibbolethBackgroundLoginSuccessful()
